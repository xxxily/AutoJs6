...
في الجدول، يتم احتساب البيانات في العمود `فترة الصيانة النشطة` التي تحتوي على أقواس زاوية (`<>`) حتى 10 June 2026.
...
* تم ترقية محرك [Rhino](https://github.com/mozilla/rhino/) من [v1.7.7.2](https://github.com/mozilla/rhino/releases/tag/Rhino1_7_7_2_Release) إلى [v2.0.0-SNAPSHOT](http://rhino.autojs6.com/blob/master/gradle.properties#L3) (تم التحديث في 25 December 2025)
...
* `إضافة جديدة` إضافة مركز موحد لحالة القدرات وفحوصات قبل التشغيل مع التحقق من القدرات ومداخل الأذونات وملاحظات المخاطر وملفات قدرات المشروع
* `إضافة جديدة` إضافة DSL موثوق للأتمتة وتشخيصات لقطات واجهة المستخدم وجلسات لقطات الشاشة ومسار مدمج لإدراك الشاشة
* `إضافة جديدة` إضافة واجهات API مميزة ومنظمة لـ Shizuku/Root لتقليل سلاسل shell المكتوبة يدوياً وتسجيل تدقيق المخاطر
* `إضافة جديدة` إضافة سجلات تشغيل المهام وسياسات موثوقية الجدولة وعزل قدرات السكربت وتدقيق الاستدعاءات عالية المخاطر
* `إضافة جديدة` إضافة تصريحات أذونات SDK للإضافات وفحوصات سلسلة التوريد ومراقبة التصحيح عن بعد وعملاء HTTP مسماة والتنزيل القابل للاستئناف و IPC محلي و typed Broadcast
* `إضافة جديدة` إضافة فحوصات ما قبل التغليف وحزم تشخيص inrt والمرحلة الثانية من AI Copilot ومكتبة حلول الأتمتة
* `تحسين` تحسين الاتساق بين فهارس API والوثائق المدمجة وفهارس الذكاء الاصطناعي والوثائق الخارجية، مع إضافة أمثلة قدرات ومصفوفات صيانة وقوائم قبول للانحدار

# v6.7.2

###### 2026/06/07

* `إضافة جديدة` إضافة ميزة مساعد البرمجة النصية بالذكاء الاصطناعي، ودعم كتابة النصوص وتعديلها وشرحها بمساعدة الذاء الاصطناعي في محرر الأكواد، إلخ.
* `تحسين` تحديث سير عمل GitHub Actions لاستخدام إصدارات إجراءات متوافقة مع Node 24 لإزالة تحذيرات الإهمال

# v6.7.1

###### 2026/06/07

* `إضافة جديدة` اضافة لوحة سجلات سفلية في محرر الكود, مع دعم عرض مخرجات التشغيل ومسح السجلات داخل المحرر
* `تصليح` تمت مطابقة احداث اللمس مع مساحة احداثيات الشاشة _[`issue #59`](http://issues.autojs6.com/59)_
* `تصليح` تهيئة WorkManager قبل جدولة المهام لتجنب اخطاء الجدولة _[`issue #501`](http://issues.autojs6.com/501)_
* `تصليح` الاحتفاظ بعنوان URI الاصلي عند فتح السكربتات لتحسين الحفظ اللاحق والتعرف على المسار _[`issue #545`](http://issues.autojs6.com/545)_
* `تصليح` اختصارات التطبيقات المعباة تتبع اعداد اظهار السجلات _[`issue #550`](http://issues.autojs6.com/550)_
* `تصليح` تطبيق مستوى السجل الجذري للكونسول العام على المخرجات بشكل صحيح _[`issue #540`](http://issues.autojs6.com/540)_
* `تصليح` dialogs.multiChoice يعيد الآن نتائج من نوع array _[`issue #518`](http://issues.autojs6.com/518)_ _[`issue #322`](http://issues.autojs6.com/322)_
* `تصليح` files.ensureDir ينشئ المجلدات بشكل صحيح _[`issue #539`](http://issues.autojs6.com/539)_
* `تصليح` واجهات app API تتعرف بشكل صحيح على اسماء الحزم المثبتة _[`issue #495`](http://issues.autojs6.com/495)_
* `تصليح` استعادة اعدادات OkHttp المتغيرة للتوافق _[`issue #536`](http://issues.autojs6.com/536)_
* `تصليح` تجنب تسجيل حمولة البايت البعيدة كاملة في السجلات _[`issue #523`](http://issues.autojs6.com/523)_
* `تصليح` انتظار timeout في disposable يعود بشكل صحيح عند الاشارة _[`issue #503`](http://issues.autojs6.com/503)_
* `تصليح` احداثيات الايماءات تستخدم الآن مقياس الشاشة على محور Y _[`issue #541`](http://issues.autojs6.com/541)_
* `تصليح` اصلاح خطأ بناء متعلق بلوحة السجلات السفلية _[`issue #543`](http://issues.autojs6.com/543)_
* `تصليح` ConcatReader.close يواصل اغلاق بقية reader عند فشل احدى عمليات الاغلاق
* `تحسين` تحديث بيانات توافق اصدارات AGP في Gradle Settings
* `تحسين` تحديث تعليق وتاريخ اصدار Temurin

##### للاطلاع على المزيد من سجل الإصدارات، راجع
...
قم بتنزيل إصدار `Android Studio Panda 2 | 2025.3.2` (حدد أحدها حسب الحاجة):
...
> ملاحظة: تاريخ الإصدار بالنسخة المذكورة أعلاه هو 3 March 2026. إذا كنت بحاجة إلى تنزيل إصدار آخر، أو إذا كان الرابط المذكور غير صالح، يمكنك زيارة [أرشيف إصدارات Android Studio](https://developer.android.com/studio/archive?hl=en).
...
اعتبارًا من 10 June 2026، الإصدار الأقصى المدعوم من JDK لمشروع AutoJs6 هو `25`.
...
تم تحديث البيانات في 4 March 2026.
