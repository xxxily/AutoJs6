...
في الجدول، يتم احتساب البيانات في العمود `فترة الصيانة النشطة` التي تحتوي على أقواس زاوية (`<>`) حتى 17 June 2026.
...
* `إضافة جديدة` إضافة وحدة JVM مستقلة capability-governance لاستخراج نموذج القدرات الأساسي مع الحفاظ على توافقه مع سجل Android وقت التشغيل
* `إضافة جديدة` إضافة بوابة release CI تشغل connected smoke suite عند توفر جهاز، وتحافظ على بوابة تجميع androidTest مع سبب تخط واضح عند عدم وجود جهاز متصل
* `تصليح` إصلاح workflow الإصدار لإنشاء ورفع ملفات APK من app/releases بأسماء تحتوي على digest بدلاً من مخرجات APK الخام
* `تحسين` تعزيز تنفيذ السكربتات الخارجية و WebView bridge ودورة حياة OCR/native وتدقيق تحرير الموارد وحوكمة SDK الإضافات وبوابات سلسلة توريد الإصدار لإكمال عناصر تقييم البنية P0-P2
...
اعتبارًا من 17 June 2026، الإصدار الأقصى المدعوم من JDK لمشروع AutoJs6 هو `25`.
