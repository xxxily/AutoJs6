...
표에서 `활성 유지보수 기간` 열에 꺾쇠괄호 (`<>`) 가 포함된 데이터는 통계 기준일이 2026 년 6 월 17 일 임을 나타냅니다.
...
* [Rhino](https://github.com/mozilla/rhino/) 엔진을 [v1.7.7.2](https://github.com/mozilla/rhino/releases/tag/Rhino1_7_7_2_Release) 에서 [v2.0.0-SNAPSHOT](http://rhino.autojs6.com/blob/master/gradle.properties#L3) 로 업그레이드 (2025 년 12 월 25 일 업데이트됨)
...
* `추가` 독립 JVM 모듈 capability-governance 를 추가하여 기능 거버넌스 핵심 모델을 분리하고 Android 런타임 레지스트리와의 정합성을 유지
* `추가` 연결된 기기가 있을 때 connected smoke suite 를 실행하는 release CI 게이트를 추가하고, 기기가 없을 때는 androidTest 컴파일 게이트와 명시적 건너뜀 사유를 유지
* `수정` release workflow 가 원본 APK 출력 대신 digest 이름이 붙은 app/releases APK 를 생성하고 업로드하도록 수정
* `개선` 외부 스크립트 실행, WebView bridge, OCR/native 생명주기, 리소스 회수 감사, 플러그인 SDK 거버넌스, release 공급망 게이트를 강화하여 아키텍처 평가 P0-P2 항목 완료

# v6.8.0

###### 2026/06/10

* `추가` 통합 기능 상태 센터와 실행 전 점검을 추가하여 기능 검사, 권한 진입점, 위험 설명, 프로젝트 기능 매니페스트를 지원
* `추가` 신뢰성 자동화 DSL, UI 스냅샷 진단, 스크린샷 Session, 화면 인식 융합 파이프라인 추가
* `추가` 구조화된 Shizuku/Root 특권 API를 추가하여 수동 shell 문자열을 줄이고 위험 감사를 기록
* `추가` 작업 실행 기록, 스케줄러 신뢰성 정책, 스크립트 기능 샌드박스, 고위험 호출 감사 추가
* `추가` 플러그인 SDK 권한 선언, 공급망 검증, 원격 디버깅 관측성, 명명된 HTTP client, 이어받기 다운로드, 로컬 IPC, typed Broadcast 추가
* `추가` 패키징 사전 점검, inrt 진단 번들, AI Copilot 2단계, 자동화 솔루션 라이브러리 추가
* `개선` API 인덱스, 내장 문서, AI 인덱스, 외부 문서의 일관성을 개선하고 기능 예제, 유지보수 매트릭스, 회귀 승인 체크리스트를 보강
...
`Android Studio Panda 2 | 2025.3.2` 버전을 다운로드하십시오 (필요에 따라 선택):
...
> 메모: 상기 버전 출시 날짜는 2026 년 3 월 3 일입니다. 다른 버전을 다운로드하거나 상기 링크가 만료되었을 경우, [Android Studio 릴리스 아카이브](https://developer.android.com/studio/archive?hl=en) 페이지를 방문하십시오.
...
2026 년 6 월 17 일 기준으로, AutoJs6이 지원하는 최대 JDK 버전은 `25` 입니다.
